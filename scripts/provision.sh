...
function deploy() {
  ...
  oc $ARG_OC_OPS policy add-role-to-user edit system:serviceaccount:tasks-dev:jenkins -n tasks-prod
  oc $ARG_OC_OPS policy add-role-to-user edit system:serviceaccount:tasks-dev:jenkins -n tasks-test
  ...
  if [ $LOGGEDIN_USER == 'system:admin' ] ; then
    ...
    oc $ARG_OC_OPS adm pod-network join-projects --to=tasks-dev tasks-prod tasks-test >/dev/null 2>&1
  fi

  sleep 2

  oc new-app jenkins-ephemeral -n tasks-dev

  sleep 2

  local template=https://raw.githubusercontent.com/$GITHUB_ACCOUNT/openshift-cd-demo/$GITHUB_REF/cicd-template.yaml
  echo "Using template $template"
  oc $ARG_OC_OPS new-app -f $template --param DEV_PROJECT=tasks-prod --param STAGE_PROJECT=tasks-test --param=WITH_CHE=$ARG_DEPLOY_CHE --param=EPHEMERAL=$ARG_EPHEMERAL -n tasks-dev 
}
...
case "$ARG_COMMAND" in
    delete)
        echo "Delete demo..."
        oc $ARG_OC_OPS delete project tasks-prod tasks-test tasks-dev
        echo
